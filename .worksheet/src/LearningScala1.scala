object LearningScala1 {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(126); 
  ...
  val pi : Double = 3.14159265;System.out.println("""pi  : Double = """ + $show(pi ));$skip(70); 
  // add "f" at the end
  val piSinglePrecision : Float = 3.14159265f;System.out.println("""piSinglePrecision  : Float = """ + $show(piSinglePrecision ));$skip(61); 
  // add "l" at the end
  ...
  val isLesser = 1 < 2;System.out.println("""isLesser  : Boolean = """ + $show(isLesser ));$skip(93); 
  // execute both of them, use this one for security
  val impossible = isGreater & isLesser;System.out.println("""impossible  : Boolean = """ + $show(impossible ));$skip(110); 
  // execute the first one if possible, use this one for performance
  ...
  val bestCaptain: String = "Picard";System.out.println("""bestCaptain  : String = """ + $show(bestCaptain ));$skip(86); 
  // compare content of the two strings
  ...
  val doublePi = pi * 2;System.out.println("""doublePi  : Double = """ + $show(doublePi ));$skip(47); 
  println(f"This is double pi: $doublePi%.3f")}
}
